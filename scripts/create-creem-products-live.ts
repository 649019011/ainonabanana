 ...
 * Creem 生产环境产品创建脚本
 *
 * 此脚本通过 Creem 生产环境 API 创建 Nano Banana 的订阅产品
 * 运行：npx tsx scripts/create-creem-products-live.ts
 *
 * 重要：此脚本会创建生产环境的产品，请确保你已切换到 Live Mode
 */
...
async function createProduct(product: CreemProduct, apiKey: string): Promise<CreateProductResponse> {
  // 生产环境 API URL
  const apiUrl = 'https://api.creem.io/v1/products'
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.log('')
    console.log('请先获取生产环境的 API Key:')
    console.log('1. 访问 https://www.creem.io/sign-in 登录')
    console.log('2. 切换到 Live Mode')
    console.log('3. 复制生产环境的 API Key（以 creem_live_ 开头）')
    console.log('4. 更新 .env.local 中的 CREEM_API_KEY')
    console.log('')
    console.log('或者直接运行: CREEM_API_KEY=creem_live_xxx npx tsx scripts/create-creem-products-live.ts')
    process.exit(1)
  }

  // 检查是否是生产环境 Key
  if (!apiKey.startsWith('creem_live_')) {
    console.warn('⚠️  警告: 当前的 API Key 不是生产环境 Key')
    console.log('   生产环境 Key 应该以 creem_live_ 开头')
    console.log('   当前 Key 前缀:', apiKey.substring(0, 10))
    console.log('')
    const confirm = process.env.CONFIRM_TEST_KEY
    if (!confirm) {
      console.log('如果确实要使用当前 Key，请设置环境变量 CONFIRM_TEST_KEY=1')
      process.exit(1)
    }
  }

  console.log('🔧 使用生产环境 API')
  console.log('📡 API URL: https://api.creem.io')
  console.log('🔑 API Key:', apiKey.substring(0, 15) + '...')
  console.log('')
  console.log('⚠️  即将在生产环境创建产品，这将创建真实可用的产品！')
  console.log('')
  ...
  for (const product of products) {
    try {
      ...
      const result = await createProduct(product, apiKey)
  ...
  if (results.length > 0) {
    ...
    console.log('='.repeat(60))
    console.log('')
    console.log('请将以下内容更新到你的 .env.local 文件中:')
    console.log('')
    ...
    console.log('')
    console.log('同时更新 CREEM_API_URL 为生产环境:')
    console.log('CREEM_API_URL=https://api.creem.io/v1')
    console.log('')
    console.log('='.repeat(60))
